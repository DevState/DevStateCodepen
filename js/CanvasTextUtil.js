...
(function (window){
    ...
    Sakri.CanvasTextUtil.getFontSizeForWidth = function(string, fontProps, width, canvas, fillStyle){
        if(!canvas){
            var canvas = document.createElement("canvas");
        }
        if(!fillStyle){
    ...
    //returns the biggest font size that best fits into rect DOESN'T WORK
    ...
    //TODO, rewrite a third one which uses getTransparentPixel for the perfect size
    //returns the biggest font size that best fits into rect
    Sakri.CanvasTextUtil.getFontSizeForRect2 = function(string, fontProps, rect, canvas, fillStyle){
        if(!canvas){
            var canvas = document.createElement("canvas");
        }
        if(!fillStyle){
        ...
        var width = context.measureText(string).width;
        if(width < rect.width){
            while(context.measureText(string).width < rect.width && copy.fontSize < rect.height){
                copy.fontSize++;
                context.font = copy.getFontString();
            }
        }else if(width > rect.width){
